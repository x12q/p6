...
class PasteRangeRMImp @Inject constructor(
    ...
    override fun pasteRange(request: PasteRangeRequest): PasteRangeResponse? {
        val wsState = appState.getWorkbookState(request.wbWs.wbKey)?.getWorksheetState(request.wbWs.wsName)
        if(wsState!=null){
            val target = RangeId(
    ...
    override fun pasteRange2(request: PasteRangeRequest2): PasteRangeResponse? {
        val wsState = appState.getWorkbookState(request.wbKey)?.getWorksheetState(request.wsName)
        if(wsState!=null){
            val target = RangeId(
