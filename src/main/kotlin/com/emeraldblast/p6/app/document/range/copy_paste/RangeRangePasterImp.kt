...
/**
 * Paste a range with target being a range
 */
class RangeRangePasterImp @Inject constructor(
    ...
    companion object {
        ...
        fun paste(rangeCopy: RangeCopy, target: RangeId, wb: Workbook): Result<Workbook, ErrorReport> {
    ...
    private fun getRangeCopyFromClipboard(wbKey: WorkbookKey, wsName: String): RangeCopy {
        val translator = appState.translatorContainer.getTranslator(wbKey, wsName)
    ...
    fun paste(rangeCopy: RangeCopy, target: RangeId): Result<Workbook, ErrorReport> {
        val rt = appState.getWorkbookRs(target.wbKey).andThen { wb ->
            paste(rangeCopy, target, wb)
        }
        return rt
    }
}
